...
impl<T> FastVec<T> {
    ...
    pub fn remove(&mut self, i: usize) {
        ...
        unsafe {
            let remove_ptr = self.ptr_to_data.add(i);

            // Run the destructor for the element being removed.
            ptr::drop_in_place(remove_ptr);

            // Shift remaining elements left to fill the gap.
            let elems_to_move = self.len - i - 1;
            if elems_to_move > 0 {
                ptr::copy(remove_ptr.add(1), remove_ptr, elems_to_move);
            }
